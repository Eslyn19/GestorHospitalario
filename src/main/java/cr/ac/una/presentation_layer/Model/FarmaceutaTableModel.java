...
public class FarmaceutaTableModel extends AbstractTableModel implements IServiceObserver<Farmaceuta> {
    private final String[] cols = {"ID", "Nombre", "Apellido"};
    ...
    public Farmaceuta getAt(int row) { return (row >= 0 && row < rows.size()) ? rows.get(row) : null; }
    ...
    @Override public Object getValueAt(int rowIndex, int columnIndex) {
        Farmaceuta farmaceuta = rows.get(rowIndex);
        switch (columnIndex) {
            case 0: return farmaceuta.getID();
            case 1: return farmaceuta.getNombre();
            case 2: return farmaceuta.getApellido();
            default: return null;
        }
    }
    ...
    // Observer
    @Override
    public void onDataChanged(ChangeType type, Farmaceuta farmaceuta) {
        switch (type) {
            case CREATED: {
                rows.add(farmaceuta);
                int i = rows.size() - 1;
            ...
            case UPDATED: {
                int i = indexOf(farmaceuta.getID());
                if (i >= 0) {
                    rows.set(i, farmaceuta);
